...
void round_chabiao(u32 x[], u32 RK[]) {
	...
	for (i = 0; i < 32; i++) {
		...
		x[(i + 4) % 4] = x[i % 4] ^ BOX0[(u8)temp >> 24] ^ BOX1[(u8)temp >> 16] ^ BOX2[(u8)temp >> 8] ^ BOX3[(u8)temp];
	}
}
void round_simd(u32 x[], u32 RK[]) {
	short i;
	__m256i x0, x1, x2, x3, rk,vindex;
	for (i = 0; i < 32; i++) {
		...
		x0 = _mm256_i32gather_epi32((int*)
			(x + 4 * 0), vindex, 4);
		x1 = _mm256_i32gather_epi32((int*)
			(x + 4 * 1), vindex, 4);
		x2 = _mm256_i32gather_epi32((int*)
			(x + 4 * 2), vindex, 4);
		x3 = _mm256_i32gather_epi32((int*)
			(x + 4 * 3), vindex, 4);
		rk = _mm256_set1_epi32(RK[i]);
		x1 = _mm256_xor_si256(x1, x2);
		x3 = _mm256_xor_si256(rk, x3);
...
/*
    simd加密算法p_num
	每次吞入 8组明文
	参数：	u32 X[32]：明文    u32 RK[32]：轮密钥    u32 Y[32]：密文，保存结果
	返回值：无
*/
...
/*
	...
	返回值：无
*/
void encryptSM4_zhankai(u32 x[], u32 RK[], u32 y[]) {
	round_zhankai(x, RK);
	reverse(x, y);
}
/*
	多线程加密算法p_num
	参数： 	u32 X[]：密文起始地址    u32 RK[32]：轮密钥    u32 Y[]：明文起始地址，保存结果   p_num 明文组数
	返回值：无
*/
...
void encryptSM4_pthread() {
	pthread_t tids[NUM_THREADS];
	for (int i = 0; i < NUM_THREADS; ++i)
	{
		int ret = pthread_create(&tids[i], NULL, sub_thread, (void*)i);
	}
	//线程结束
	for (int i = 0; i < NUM_THREADS; i++)
	{
...
int main(void) {
	 ...
	 // 临时变量 
	X = (u32)malloc (NUM_P*4*sizeof(u32));
	Y = (u32)malloc(NUM_P * 4 * sizeof(u32));
	//随机数种子
	srand((unsigned)time(NULL));
	//给矩阵赋值
	for (int i = 0; i < NUM_P * 4; i++)
	{
		X[i] = (u32)rand() % 11;
	}
	set_RK(MK, K, RK);
	...
	long long c1, c2;
	double dff;
	QueryPerformanceFrequency(&larger_integer);
	dff = larger_integer.QuadPart;
	double  nTime;

	QueryPerformanceCounter(&larger_integer);
	c1 = larger_integer.QuadPart;
	for (int i = 0; i < NUM_P; i++)
		encryptSM4(X + 4 * i, RK, Y + 4 * i);
	QueryPerformanceCounter(&larger_integer);
	...
	c1 = larger_integer.QuadPart;
	for (int i = 0; i < NUM_P; i++)
		encryptSM4_zhankai(X + 4 * i, RK, Y + 4 * i);
	QueryPerformanceCounter(&larger_integer);
	c2 = larger_integer.QuadPart;
	nTime = (c2 - c1) / dff;
	printf("%f", nTime);
	...
	QueryPerformanceCounter(&larger_integer);
	c1 = larger_integer.QuadPart;
	for (int i = 0; i < NUM_P; i=i+8)
		encryptSM4_simd(X + 4 * i, RK, Y + 4 * i);
	QueryPerformanceCounter(&larger_integer);
	c2 = larger_integer.QuadPart;
	nTime = (c2 - c1) / dff;
	printf("%f", nTime);
